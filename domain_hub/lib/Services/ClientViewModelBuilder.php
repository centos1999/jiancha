...
class CfClientViewModelBuilder
{
    public static function build(int $userId): array
    {
        ...
        $clientDeleteMode = strtolower(trim((string) ($moduleSettings['client_domain_delete_mode'] ?? '')));
        if (!in_array($clientDeleteMode, ['disabled', 'allow_all', 'never_had_dns_only', 'no_current_dns_only'], true)) {
            $clientDeleteMode = cfmod_setting_enabled($moduleSettings['enable_client_domain_delete'] ?? '0')
                ? 'never_had_dns_only'
                : 'disabled';
        }
