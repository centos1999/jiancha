...
class CfClientActionService
{
    public static function process(array $globals): array
    {
        ...
        $clientDeleteMode = strtolower(trim((string) ($module_settings['client_domain_delete_mode'] ?? '')));
        if (!in_array($clientDeleteMode, ['disabled', 'allow_all', 'never_had_dns_only', 'no_current_dns_only'], true)) {
            $clientDeleteMode = !empty($clientDeleteEnabled) ? 'never_had_dns_only' : 'disabled';
        }
