...
(function () {
    ...
    // 避免模态框被容器层叠上下文/overflow 影响导致“只有遮罩、看不到对话框”
    // 统一将 AI 模态框挂到 body 下，由 Bootstrap 按标准层级管理。
    ...
    // 兜底清理：防止异常情况下残留 backdrop 导致整页无法点击
    ...
    try {
        ...
        if (currentUrl.searchParams.get('open_help_ai') === '1') {
            openBtn.click();
            currentUrl.searchParams.delete('open_help_ai');
