...
<?php if ($hasAnyFeature): ?>
    <div class="row g-3">
        ...
        <?php if (!empty($dnsUnlockFeatureEnabled)): ?>
            <div class="col-md-6" style="order: 9;">
                <div class="card border-0 shadow-sm h-100">
                    <div class="card-body d-flex flex-column">
        ...
        <?php if (!empty($domainPermanentIncentiveEnabled)): ?>
            <div class="col-md-6" style="order: 4;">
                <div class="card border-0 shadow-sm h-100 border-warning-subtle">
                    <div class="card-body d-flex flex-column">
                        <h6 class="card-title"><i class="fas fa-bolt text-warning me-2"></i><?php echo $featureText('cfclient.feature.permanent_incentive.title', '域名永久激励中心（限时）', 'Domain Permanent Incentive Center (Limited Time)'); ?></h6>
        ...
        <?php if ($hasRootdomainInvite): ?>
            <div class="col-md-6" style="order: 10;">
                <div class="card border-0 shadow-sm h-100">
                    <div class="card-body d-flex flex-column">
        ...
        <?php if ($orphanDnsCleanupEnabled): ?>
            <div class="col-md-6" style="order: 6;">
                <div class="card border-0 shadow-sm h-100 border-danger-subtle">
                    <div class="card-body d-flex flex-column">
                        <h6 class="card-title"><i class="fas fa-broom text-danger me-2"></i><?php echo $featureText('cfclient.feature.orphan_cleanup.title', '域名记录冲突清理', 'Domain DNS Conflict Cleanup'); ?></h6>
                        <p class="text-muted small mb-2"><?php echo $featureText('cfclient.feature.orphan_cleanup.desc', '用于处理“域名添加DNS解析时提示已存在或冲突的问题。提交后将删除所选域名在云端的全部 DNS 记录。', 'Use this tool when adding DNS records shows \"already exists\" or conflict errors. Submitting will delete all DNS records for the selected domain from the provider.'); ?></p>
        ...
        <?php if ($rootVerifyEnabled): ?>
            <div class="col-md-6" style="order: 7;">
                <div class="card border-0 shadow-sm h-100">
                    <div class="card-body d-flex flex-column">
                        <h6 class="card-title"><i class="fas fa-shield-alt text-success me-2"></i><?php echo $featureText('cfclient.feature.root_verify.title', '根域名验证(测试版)', 'Root Domain Verification (Beta)'); ?></h6>
        ...
        <?php if ($githubStarRewardEnabled): ?>
            <div class="col-md-6" style="order: 8;">
                <div class="card border-0 shadow-sm h-100">
                    <div class="card-body d-flex flex-column">
